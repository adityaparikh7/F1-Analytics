          ...
          >
            ...
            <polyline
              points={renderData.pts.map(p => {
                const x = pad + ((p.x! - renderData.minX) / (renderData.maxX - renderData.minX || 1)) * (svgSize - pad * 2);
                const y = pad + ((renderData.maxY - p.y!) / (renderData.maxY - renderData.minY || 1)) * (svgSize - pad * 2);
                return `${x},${y}`;
              }).join(' ')}
              fill="none"
